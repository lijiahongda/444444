<template>
	<div class="app-container">
		...
		<el-form style="margin-bottom:20px;" :model="form" ref="form" :inline="true" class="demo-form-inline">
			<el-form-item>
				<el-date-picker v-model="form.act_starttime" type="datetime" placeholder="开始时间" value-format="yyyy-MM-dd HH:mm:ss">
				</el-date-picker>
			</el-form-item>
			<el-form-item>
				<el-date-picker v-model="form.act_endtime" type="datetime" placeholder="结束时间" value-format="yyyy-MM-dd HH:mm:ss">
				</el-date-picker>
			</el-form-item>
			<el-form-item>
			...
			<el-form-item>
				...
				<router-link to="/operate/gropAdd">
					<el-button type="primary">新增拼团</el-button>
				</router-link>
				<router-link to="/shopMall-order">
					<el-button type="primary">拼团订单</el-button>
				</router-link>
			</el-form-item>
		</el-form>
		<el-table v-if="!loading" :data="list" border stripe :fit="true" style="width: 100%">
			...
			<el-table-column prop="sort" label="推广渠道" align="center">
				<template slot-scope="scope">
					<span v-html="scope.row.pop_channels"></span>
				</template>
			</el-table-column>
			<el-table-column prop="act_name" label="分享标题" align="center"></el-table-column>
			...
			<el-table-column prop="team_price" label="商品低价" align="center"></el-table-column>
			...
			<el-table-column prop="open_count" label="组团次数" align="center"></el-table-column>
			<el-table-column prop="success_count" label="成团次数" align="center"></el-table-column>
			<el-table-column prop="" label="上线状态" align="center">
				<template slot-scope="scope">
					<span v-if='scope.row.status == 0' style="color: #f00;">下线</span>
					<span v-else style="color: green;">上线</span>
				</template>
			</el-table-column>
			<el-table-column prop="act_status" label="活动状态" align="center"></el-table-column>
			<el-table-column fixed="right" prop="refundTotal" width="270px" label="操作" align="center">
				<template slot-scope="scope">
					<el-button @click="updateBargainIsOnline(scope.$index, scope.row,list)" size="mini"><span v-html='scope.row.status == 0 ? "上线" : "下线"'></span></el-button>
					<el-button size="mini" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
					<el-button size="mini" type="danger" @click="Record(scope.row.id)">活动记录</el-button>
				</template>
			</el-table-column>
		</el-table>
		<div v-if="!loading" class="pagination-container" style="text-align:right;">
			<el-pagination :current-page="form.page" :page-sizes="[10]" :page-size="10" :total="total" background layout="total, sizes, prev, pager, next, jumper" @current-change="handleCurrentChange" />
		</div>
	</div>
</template>
<script>
	// import hotelCharts from './hotelCharts' // 图表
import { combinationsList, teamList,updateCombinationSort,updateDownCombination } from '@/api/shops' // 接口
    ...
    methods: {
      // 查看记录
          ...
          teamList(data).then((res) => {
            ...
            console.log(error)
				...
				updateBargainIsOnline(idx,row,list){
					...
					updateDownCombination(obj).then(res=>{
						if(res.data.code == 200){
							let text = '';
							if(row.status == 0){
								list[idx].status = 1;
        ...
        getList() {
          ...
          _this.loading = true
          let data = this.form;
          combinationsList(data).then(res => {
            if (res.data.code === 200){
      ...
      changeorder (index,gid) {
        ...
        handleEdit(index, row) {
          this.$router.push({ path: '/operate/gropAdd', query: { id: row.id }})
        },
        //广告删除
