  ...
  <div class="lottery">
    <el-button type="primary"
               @click="addLottery">新增领券活动</el-button>
	...
	<script>
import { configList, deleteCouponCollectionCenter } from "@/api/order";
export default {
  name: "lottery",
  data () {
    return {
  ...
  methods: {
    ...
    addLottery () {
      this.$router.push({ path: "/operate/vouchersCenter" });
    },
    edit (row) {
      this.$router.push({ path: "/operate/vouchersCenter", query: { id: row.id } });
    },
    delRow (row) {
