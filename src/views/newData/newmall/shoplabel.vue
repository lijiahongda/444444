	...
	<div class="app-container">
		<el-form ref="form" label-width="120px" :inline="true">
			<el-form-item label="活动名称：">
				<el-col>
					<el-input v-model="label_name" placeholder="请输入活动名称" style="width:300px;" />
				</el-col>
			</el-form-item>
			<el-form-item>
			...
			<el-table v-if="!switchChart" :data="list" :fit="true" border style="width: 100%;">
				<el-table-column fixed prop="" label="活动名称" align="center">
					<template slot-scope="{row}">
						<el-input v-if="row.edit"  v-model="row.label_name" class="edit-input" size="small" />
						<span v-else>{{ row.label_name }}</span>
					</template>
				</el-table-column>
...
<script>
	...
	import {getLabels,addLabel,updateLabel,deleteLabel} from '@/api/cust'
...
export default {
    ...
    methods: {
        // 列表数据
        getList() {
			getLabels({page:`${this.pageIndex}`}).then(res=>{
				if(res.data.code==200){
					let dataList = res.data.data.result;
						this.list = dataList.map((item)=>{
							this.$set(item, 'edit', false);
							item.label_name_new = item.label_name;
							return item
						})
		...
		onConfirm(type,row){//type=1添加 type=2编辑
			...
			if(type == 2){
				row.edit = false;
				obj.label_name = row.label_name;
				obj.lid = row.lid;
				updateLabel(obj).then(res=>{
					if(res.data.code==200){
						this.$message({
							message: '保存成功',
							type: 'success'
				...
				if(this.label_name != ""){
					obj.label_name = this.label_name;
				}else{
					this.$message({
						message:"请输入商品标签名称",
						type:"error"
		...
		onDel(row){
			this.$confirm('此操作将永久删除, 是否继续?', '提示', {
			  ...
			  type: 'warning'
				...
				deleteLabel({lid:`${row.lid}`}).then(res=>{
					if(res.data.code==200){
						  this.$message({
							  message:"删除成功",
							  type:"success"
