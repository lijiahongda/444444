      ...
      <el-table :data="materialList"
        ...
        <el-table-column label="操作"
          ...
          <template slot-scope="scope"
            ...
            <el-button type="text"
                       size="small"
                       @click="getOfficialMember(scope.row.id,scope.$index)">编辑</el-button>
          </template>
        </el-table-column>
      </el-table>
...
import {
  getOfficialMembers,//列表
  getOfficialMember,//单个
  delOfficialMember,//删除
  upOfficialMember,//审核
  addOfficialMember
} from '@/api/shops'
export default {
  ...
  methods: {
    ...
    DelMaterial (id, index) {
      this.$confirm('确定要删除吗?', '提示', {
        ...
        delOfficialMember(obj).then(res => {
          if (res.data.code == 200) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.materialEdit) {//编辑
            ...
            addOfficialMember(obj).then(res => {
              if (res.data.code == 200) {
                this.$message({
                  message: "保存成功",
                  type: "success"
