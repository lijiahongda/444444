<template>
    <div class="app-container">
      <el-table v-loading="loading" v-if="!switchChart" :data="list" border :fit="true" style="width: 100%">
        <el-table-column prop="threePartner" label="渠道名称" align="center"></el-table-column>
...
import { mallchannelList } from '@/api/hotelOrder' // 接口
...
export default {
    ...
    methods: {
        ...
        getList() {
		  this.loading = true
          mallchannelList(this.searchCondition).then(res => {
		...
		goToInfo(row){
			this.$router.push({
				path:'/mall-lists/mall-order-channel-info',
				query:{
					type: row.type == 0 ? '自营' : row.type,
					name: row.threePartner,
					title: '供应商'
				}
			})
		}
