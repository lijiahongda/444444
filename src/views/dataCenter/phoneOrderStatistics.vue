...
<template>
    <div class="app-container">
        ...
        <el-table v-if="!switchChart" :data="list" :fit="true" border style="width: 98%">
			...
			<el-table-column prop="" label="充值收入" align="center">
			    <template slot-scope="scope">
...
<script>
import { getCurrentMonthRecharge,getRechargeDetail} from '@/api/order' // 接口
...
export default {
    ...
    methods: {
        // 列表数据
        getList() {
            getCurrentMonthRecharge(this.searchCondition).then(res => {
         ...
         getRechargeDetail() {
            getRechargeDetail(this.searchCondition2).then(res => {
                if (res.data.code === 200){
