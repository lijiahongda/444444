...
<template>
    <div class="app-container">
        ...
        <el-table v-if="!switchChart" :data="list" :fit="true" border style="width: 98%">
			...
			<el-table-column prop="" label="充值收入" width="110">
			    <template slot-scope="scope">
			        {{scope.row.totalAmount ? scope.row.totalAmount : '0' }}
			    </template>
			</el-table-column>
			<el-table-column prop="" label="补贴金额" align="center">
			    <template slot-scope="scope">
        ...
        <el-table v-if="!switchChart" :data="list2" :fit="true" border style="width: 98%;margin-top:30px">
            ...
            <el-table-column prop="" label="充值次数" align="center">
                <template slot-scope="scope">
                    {{scope.row.count ? scope.row.count : '-' }}
                </template>
            </el-table-column>
			<el-table-column prop="" label="充值总额" align="center">
			    <template slot-scope="scope">
...
<script>
import { getVideoMonthRecharge,getVideoRechargeDetail} from '@/api/order' // 接口
...
export default {
    ...
    mounted() {
        this.getList();
        this.getVideoRechargeDetail();
    },
    methods: {
        // 列表数据
        getList() {
            getVideoMonthRecharge(this.searchCondition).then(res => {
         ...
         getVideoRechargeDetail() {
            getVideoRechargeDetail(this.searchCondition2).then(res => {
                if (res.data.code === 200){
