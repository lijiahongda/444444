	...
	<div class="app-container">
         ...
         <el-dialog title="" :visible.sync="dialogForm" @close="closeBox">
			<el-form :model="form" ref="form">
				<el-form-item label="名称" :label-width="formLabelWidth"  prop="remark">
                    <el-input v-model="form.serviceTypeName" placeholder="请在20字以内" style="width: 450px;" />
				</el-form-item>
				<el-form-item label="类型" :label-width="formLabelWidth"  prop="remark">
				    <!-- <el-input v-model="form.type" :value="form.type" style="width: 450px;" /> -->
					<el-radio-group v-model="form.type">
					    <el-radio v-model="form.type" label="2">普通退货原因</el-radio>
					    <el-radio v-model="form.type" label="3">大礼包退货原因</el-radio>
					</el-radio-group>
				</el-form-item>
			</el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 列表数据
      getList() {
		  let data={type:this.type,searchCondition:this.searchCondition}
      ...
      add() {
        if(this.form.id!='')
        {
            var data={id :this.form.id,serviceTypeName :this.form.serviceTypeName,type:this.form.type}
      ...
      up(id)
      {
		...
		// 	type:this.type
		// };
        // getRefundServiceType(datas).then(res => {
